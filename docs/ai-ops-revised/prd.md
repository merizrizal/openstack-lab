...
7. Running a local LLM on the assistant VM is deferred from the first implementation. A local runtime such as Ollama would require separate capacity planning for GPU, RAM, and storage; model download, update, and supply-chain controls; inference-service deployment and health monitoring; runtime isolation; and model quality/latency evaluation.
...
28. The first model-enabled path may use a remote Codex runtime rather than a local LLM to avoid local-model hardware and lifecycle operations. It must remain separately approved, read-only, advisory-only, and bounded by opaque authentication, least-privilege egress, and no private-provider-protocol handling.
29. A future local-model implementation may replace the remote model boundary only after separate approval of its hardware capacity, model lifecycle, service isolation, supply-chain, and quality/latency validation requirements.
