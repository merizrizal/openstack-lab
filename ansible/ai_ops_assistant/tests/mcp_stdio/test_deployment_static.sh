...
for path in "$role_defaults" "$role_tasks" "$artifact_root/aiops_assistant_mcp_stdio_server.py" \
...
if grep -nE 'ansible\.builtin\.(shell|command|raw|expect|package|pip)|systemd|service|listener|socket|firewall|route|client registration|state: (started|absent)|enabled: true|hosts: all|/opt/openstack-ai-ops/' \
  "$role_defaults" "$role_tasks" "$config" "$requirements_in"; then
  fail "activation, network, package-install, removal, or historical-runtime behavior found"
fi

if [[ -e "$repo_root/ansible/ai_ops_assistant/playbook_deploy_mcp_stdio.yml" ]]; then
  fail "local-stdio deployment playbook is outside this static-only micro-chunk"
fi

"${PYTHON_BIN:-/home/meriz/Documents/PyEnv/myEnv/bin/python}" - "$config" "$requirements_in" <<'PY'
import json
import sys
...
"${PYTHON_BIN:-/home/meriz/Documents/PyEnv/myEnv/bin/python}" -m py_compile \
